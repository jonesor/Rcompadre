...
as_cdb <- function(from) {
  ...
  if (!all(c("metadata", "matrixClass", "mat") %in% names(from))) {
    stop("This doesn't appear to be an old compadre data object. It does ",
      "not contain all of the components 'metadata', 'mat', and ",
      "'matrixClass' required to generate a CompadreDB ",
      "object. See ?CompadreDB",
      call. = FALSE
    )
  }
  ...
  if (!matNamesMatch) {
    stop("Not all matrices in this compadre data object contain all of ",
      ...
      "CompadreMat objects. See ?CompadreMat",
      call. = FALSE
    )
  }
  ...
  # Older versions had DOI.ISBN instead of DOI_ISBN, and AnnualPeriodicity
