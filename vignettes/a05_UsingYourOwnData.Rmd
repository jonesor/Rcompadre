...
First we need to load the library, and `tidyverse` packages
...
In this example we generate a series of 2 dimension *A* matrices using a Dirichlet distribution for the *U* submatrix (i.e. to ensure the column sums of the *U* submatrix sums to 1), and a gamma distribution, to approximate the average of a Poisson process. In this case, the matrices all have the same dimension, but it is not necessary for dimension to be the same.

```{r}
nMat <- 20
Uvals <- cbind(MCMCpack::rdirichlet(nMat, c(1, 1)), MCMCpack::rdirichlet(nMat, c(1, 1)))
...
In the above example, I did not include any information about the stage definitions. Since these information were not provided, the `cdb_build_cdb` function automatically creates some information. You can view that information like this (using square brackets to choose a particular matrix model):
