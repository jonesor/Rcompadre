...
# helper: dominant eigenvalue (population growth rate)
lambda_fn <- function(mat) Re(eigen(mat, only.values = TRUE)$values[1])
```
...
CompSub$lambda <- sapply(matA(CompSub), lambda_fn)
```
...
CompFlag$lambda[no_missing] <- sapply(CompFlag$matA[no_missing], lambda_fn)
```
...
lambdaFn1 <- function(mat) {
  # check mat for missing values: if TRUE return NA, else return lambda
  ifelse(anyNA(mat), NA, lambda_fn(mat))
}
