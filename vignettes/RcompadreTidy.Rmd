...
Here's an example with the Rcomadre functions `mpm_has_active()` and `cdb_id_studies()`, each of which take a CompadreDB object and return a vector.
...
Let's say we want to know the survival probability for the first 'active' stage class (i.e. a stage that's not dormant or propagule) for every row of the database. One approch is to write our own function that takes matU and the integer index of the first active stage class, and returns the corresponding survival probability, e.g.
...
Rcompadre includes methods that allow CompdareDB objects to be used as data arguments within `ggplot()`, just like any old data frame. Below are some example plots.
...
compadre_life_expect <- Compadre %>%
  filter(MatrixComposite != "Seasonal", # filter is the dplyr version of subset
         ...
         MatrixCaptivity == "W",
         AnnualPeriodicity == "1") %>% 
  mutate(StageID = cdb_id_stage_defs(.)) %>%
