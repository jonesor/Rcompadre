...
In addition, each set of MPMs is associated with metadata including taxonomy of the species studied, the geographic location, the published source information and so on. For further details please see the two papers on COMPADRE and COMADRE (Salguero-Gomez et al. 2015, 2016), and the User Guide which is available via the website ([www.compadre-db.org](http://www.compadre-db.org)).
...
To get started you will first need to download the COMPADRE (or COMADRE) dataset from the website at [www.compadre-db.org](http://www.compadre-db.org).

When you have done that you can load it into R using the command `load`. It is usually a good idea to set up your working directory at this point. Assuming you are working in the same directory as your database file we can load the database like this:

```{r fake load the data, eval=FALSE}
load("COMPADRE_v.4.0.1.RData")
```

Alternatively, the function `fetchDB` will automatically download the latest version of COMPADRE or COMADRE from the website. For example:

```{r fake fetch data with fetchDB, eval=FALSE}
Compadre <- fetchDB('compadre')
```


However, for this vignette we will use the sample of COMPADRE data that is distributed with the package. This dataset is intended for demonstration and learning purposes only and should not be used for real analyses!

```{r load example COMPADRE data, eval=TRUE}
data(Compadre)
```

We can now ask for a summary of this object, which will tell us that it is a `CompadreDB` class `S4` object. Simply typing the name of the object (`Compadre` in this case) will give a brief summary of its contents and display the first few rows of the data (which is class `tibble`).
...
This summary tells us that we have successfully loaded the data, and that there are `r length(Compadre$mat)` matrices. The database also contains some Version information which can be accessed using the command `VersionData(Compadre)`. This includes information including the version number, date created, and link to the database user agreement.
...
If, for example, you want to check if a species is in the database you can use the `checkSpecies` function. For example, we can ask if the species _Succisa pratensis_ is present.

```{r Check species}
checkSpecies("Succisa pratensis",Compadre)
```
...
checkSpecies(spList,Compadre)
```

Optionally, the function can return a subset of the database restricted to matched species names.

```{r Check species3}
compadre_succisa<- checkSpecies("Succisa pratensis",Compadre,returnDatabase = TRUE)
compadre_succisa
```
...
# Subsetting the data based on metadata

It is often desirable to subset the data based on sets of criteria. As with a normal dataframe these datbases can be subset using `subset`. For example, I could subset to only Eudicot species as follows:
...
You can compare compadre datasets using the `compareDBs` command:

```{r compare }
compareDBs(Compadre,x)
```
...
We can flag these by adding metadata using the `cleanDB` function.

```{r cleanDB }
Compadre_flagged <- cleanDB(Compadre)
```
...
When we have a set of matrices that we want to make calculations from, we can use `lapply` to apply a function across all the matrices in the list produced by `matA`. For example, to calculate lambda from all the **A** matrices we can use the following code to obtain lambda for the matrices in the `x` object produced above. The `lapply` function returns a list so it is first necessary to coerce the list into a vector using `unlist`. Then we can examine a summary, produce a histogram etc.

```{r matrix calculations}
lambdaVals<-lapply(matA(x),popdemo::eigs,what="lambda")
