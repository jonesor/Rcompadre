...
Rcompadre includes methods for a variety of functions in the "[Tidyverse](https://tidyverse.org/)", a popular group of R packages geared toward data analysis, including [dplyr](https://dplyr.tidyverse.org/) (a grammar of data manipulation) and [ggplot2](https://ggplot2.tidyverse.org/) (a grammar of graphics). This vignette covers manipulation of `CompadreDB` objects using `dplyr`, and a few examples of plotting `CompadreDB` objects using `ggplot2.` Users new to these packages may wish to check out more introductory materials (e.g. the [dplyr vignette](https://CRAN.R-project.org/package=dplyr), or the [Data Visualization chapter](https://r4ds.had.co.nz/data-visualisation.html) of _R for Data Science_).
...
Let's say we want to calculate the population growth rate for every matrix in the database. We can use base R's `eigen()` to extract the dominant eigenvalue (lambda), but it can only take a single matrix at a time.

To apply it to a list of matrices we can use `sapply()`. But first we need to remove matrices with missing values, because these will cause `eigen()` to fail.

```{r}
lambda_fn <- function(m) Re(eigen(m, only.values = TRUE)$values[1])
...
compadre_lambda <- Compadre %>%
  ...
  mutate(lam = sapply(mat_A, lambda_fn))
```
...
If the optional `maps` package is not installed, the world map example below
will be shown but not executed during vignette rendering.

```{r warning=FALSE, fig.width = 6, fig.height = 4, eval = has_maps}
#| fig.alt: >
