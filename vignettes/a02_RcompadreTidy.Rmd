...
Users may also wish to take advantage of pipes. Pipes are available in the [magrittr](https://magrittr.tidyverse.org/) package (`%>%`), and (since R version 4.1.0) in base R (`|>`). Pipes pass an object on the left to the first argument of a function on the right and the two types (`%>%` and `|>`) are interchangeable.
