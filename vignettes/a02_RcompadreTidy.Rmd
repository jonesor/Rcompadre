...
Let's say we want to calculate the population growth rate for every matrix in the database. We can use the `lambda()` function in the [popbio](https://CRAN.R-project.org/package=popbio) library, but it can only take a single matrix at a time.
