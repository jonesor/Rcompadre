...
The advantage of piping here is that we don't have to use nested functions (`subset(cdb_flag())`), and we don't have to create object names for every intermediate step in our analysis. An additional alternative is to use `filter()` instead of `subset()`.
