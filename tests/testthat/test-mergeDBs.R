...
test_that("mergeDBs works correctly", {
  
  db1 <- Compadre[1:50,]
  db2 <- Compadre[51:100,]
  dbm1 <- mergeDBs(db1, db2)
  ...
  dbm2 <- mergeDBs(db1, db2)
  expect_true(is.na(dbm2@version$Version))
})

test_that("mergeDBs warns and fails gracefully", {
  
  db1 <- Compadre[1:50,]
  db2 <- Compadre[51:100,]
  expect_error(mergeDBs(db1@data, db2))
  
  db3 <- db2[,-30]
  expect_error(mergeDBs(db1, db3))
})
